 ...
 * 
 * <p>
 * Utility that generates an Excel spreadsheet. Resulting Excel file will be
 * saved under {@value #ROOT_DIRECTORY}
 * </p>
 *
 * @author Payman Salek
 * 
...
public class SpreadsheetWriterDAO {
	...
	public static List<StudentAttendance> createAttendanceReportSpreadsheet(
		...
		for (StudentAttendance sa : studentAttendanceRecords) {
			...
			char startColumn = 'H'; // First column for attendance cells. SUM
									// goes over H to (H + size - 1)
			row.createCell(columnIndex++).setCellFormula("SUM(" + startColumn + rowIndex + ":"
					+ calculateColumn(startColumn, sa.getAttendance().size()) + rowIndex + ")");
			row.createCell(columnIndex++).setCellFormula(
					"ROUND(100*E" + rowIndex + "/" + sa.getCourseOffering().getBlock().getRequiredSessions() + ", 1)");
			row.createCell(columnIndex++).setCellFormula(
					String.format("IF(F%1$d >= 90, 1.5, IF(F%1$d >= 80, 1, IF(F%1$d >= 70, 0.5, 0)))", rowIndex));
		...
		String fileName = directory + courseOffering.getCourse().getNumber() + "-"
				+ courseOffering.getFaculty().getFirstName() + courseOffering.getFaculty().getLastName() + ".xlsx";
